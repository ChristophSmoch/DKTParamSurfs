...
* GMRES_NEW: A Generalized Minimal Residual Algorithm for Solving Nonsymmetric Linear Systems.
...
*
* Walker, H. F.
* Implementations of the GMRES_NEW method.
* Comput.Phys.Comm. 53, 1989, pp. 311 - 320.
*
* Walker, H. F.
* Implementation of the GMRES_NEW Method using Householder Transformations.
...
bool gmres_new(const MatrixType & mat, const Rhs & rhs, Dest & x, const Preconditioner & precond,
  ...
  for (Index k = 1; k <= restart; ++k)
  {
    ...
    if( outputLevel > 0 ){
     ...
     std::cout << "residual = " << tol_error << std::endl;
...
/** \ingroup IterativeLinearSolvers_Module
  * \brief A GMRES_NEW solver for sparse square problems
  *
  ...
  * // fill A and b
  * GMRES_NEW<SparseMatrix<double> > solver(A);
  * x = solver.solve(b);
  ...
  * 
  * GMRES_NEW can also be used in a matrix-free context, see the following \link MatrixfreeSolverExample example \endlink.
  *
