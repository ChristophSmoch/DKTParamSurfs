#ifndef __DKTFEQUADRATURE_H
#define __DKTFEQUADRATURE_H
...
template <typename RealType, typename DomVecType, int N>
class TestQuadrature{
    
protected:
    std::vector<DomVecType> _c;
  ...
  TestQuadrature() {
        for( int x=0; x<= N; ++x )
            for( int y=0; y<=N; ++y ){
                ...
                if( (xCoord + yCoord) <= 1. ){
                    _c.push_back( DomVecType ( xCoord, yCoord ) );
                }
            }
  }
...
public:
  ...
  inline const DomVecType& getRefCoord ( int quadpoint ) const  {
    ...
    static const DomVecType c[7] = {    DomVecType ( 0., 0. ),
                                        DomVecType ( 1., 0. ),
                                        DomVecType ( 0., 1. ),
                                        DomVecType ( 1./2., 1./2. ),
                                        DomVecType ( 0., 1./2. ),
